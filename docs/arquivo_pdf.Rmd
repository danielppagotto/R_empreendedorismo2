...
Esse é o caderno de Scripts do módulo 2. Para este módulo utilizaremos os seguintes pacotes. Confira a seguir: 
...
setwd("~/Documentos Daiane/Curso R - testes")
```
...
Vamos usar o `glimpse` para visualização dos dados na base.
...
Vamos ainda adicionar e destacar mais as linhas, e retirar esse fundo cinza. 
...
## Adicionando títulos
Para adicionar título, vamos usar o `ggtitle`, além disso, iremos usar as funções `xlab` e `ylab`.
...
Muitas pesquisas utilizam o GEM associado a outras bases. Para o nosso exercício, vamos utilizar dados da Worldwide Governance Indicator (WGI) que pode ser acessada por meio da seguinte página.
